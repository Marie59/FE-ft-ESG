...
def run(params, logger):
    ...
    #credentials = ee.ServiceAccountCredentials("galaxy-te@ee-giulianolangella.iam.gserviceaccout.com","galaxy_fix_tool/key.json")
    #ee.Initialize(credentials)
    
    #prod_mode = params.get("prod_mode")
    #geojsons = json.loads(params.get("geojsons"))
    #crs = param_group.get("crs")
    prod_mode = params["data"]["params"]["prod_mode"]
    geojsons = params["data"]["params"]["geojsons"]
    crs = params["data"]["params"]["crs"]
    ...
    if prod_mode == ProductivityMode.TRENDS_EARTH_5_CLASS_LPD.value:
        ...
        # CONVERSION
        calc_traj = params["data"]["params"]["calc_traj"]
        calc_state = params["data"]["params"]["calc_state"]
        calc_perf = params["data"]["params"]["calc_perf"]
        prod_traj_year_initial = int(params["data"]["params"]["prod_traj_year_initial"])
        prod_traj_year_final = int(params["data"]["params"]["prod_traj_year_final"])
        prod_perf_year_initial = int(params["data"]["params"]["prod_perf_year_initial"])
        prod_perf_year_final = int(params["data"]["params"]["prod_perf_year_final"])
        prod_state_year_bl_start = int(params["data"]["params"]["prod_state_year_bl_start"])
        prod_state_year_bl_end = int(params["data"]["params"]["prod_state_year_bl_end"])
        prod_state_year_tg_start = int(params["data"]["params"]["prod_state_year_tg_start"])
        prod_state_year_tg_end = int(params["data"]["params"]["prod_state_year_tg_end"])
        prod_traj_method = params["data"]["params"]["trajectory_method"]
        ndvi_gee_dataset = params["data"]["params"]["ndvi_gee_dataset"]
        climate_gee_dataset = params["data"]["params"]["climate_gee_dataset"]
